...
function createStore() {
  ...
  return {
    data: { parentId: undefined, parentAgent: undefined, children: [], selectedId: undefined },
    set(patch) {
...
function createApi(ctx) {
  ...
  return {
    ...
    /**
     * Deliver a user message to a continuable child as its next FIFO turn.
     * `parent` must be the exact live parent Agent object (captured from the
     * command invocation); running children queue behind the current turn,
     * absent ones cold-resume from their persisted log.
     */
    async followup(parent, childId, text) {
      const subagents = ctx.get('subagents', false)
      if (subagents === undefined) throw new Error('subagents service unavailable')
      return subagents.followup(parent, childId, [{ type: 'text', text }], {
        source: { kind: 'user' },
      })
    },

    /** Stop a live continuable child's current turn (user authority). */
    async interrupt(parentSessionId, childId) {
      const subagents = ctx.get('subagents', false)
      if (subagents === undefined) throw new Error('subagents service unavailable')
      return subagents.interrupt(childId, { kind: 'user', parentSessionId })
    },
  }
}
...
export function apply(ctx) {
  ...
  commands.register({
    ...
    handler: async ({ agent, rawInput }) => {
      ...
      store.set({ parentId, parentAgent: agent, children, selectedId })
