...
export function foldEvent(state, event, t) {
  ...
  switch (event.type) {
    ...
    case 'user/message': {
      ...
      const key = typeof sourceKind === 'string' ? (SRC_KEYS[sourceKind] ?? null) : null
      const tag = key !== null ? t(key) : (typeof sourceKind === 'string' ? sourceKind : t('src-user'))
      state.rows.push({ kind: 'user', text: `[${tag}] ${text}`, at })
      return true
    }
    ...
    case 'tool/call': {
      ...
      state.rows.push({ kind: 'tool', text: `⚙ ${name}${args === '' ? '' : ` ${args}`}`, at })
      return true
    }
    ...
    case 'tool/result': {
      ...
      if (blockError) {
        state.rows.push({ kind: 'error', text: t('tool-result-error', { err: brief(resultText, 200) }), at })
      } else {
