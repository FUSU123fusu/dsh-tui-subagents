...
export function createFoldState() {
  return {
    ...
    sawChunks: false,
    /** Accumulated input+output+reasoning tokens from assistant/message usage. */
    tokens: 0,
  }
}

/** Sum one usage record ({input, output, think|reasoning, …}) into a scalar. */
function usageTokens(usage) {
  ...
  return pick('input') + pick('output') + (pick('think') || pick('reasoning'))
}
...
export function foldEvent(state, event, t) {
  ...
  switch (event.type) {
    ...
    case 'assistant/message': {
      state.tokens += usageTokens(data.usage)
